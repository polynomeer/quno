...
>
> 백엔드는 [../architecture/](../architecture/system-architecture.md) 문서 참고. **2026-08-26 작성 당시 이 문서가 가정한 화면 중 투표/댓글/저장/사용자 팔로우/배지/모더레이션/답변 수정 이력은 이후 Phase 11~20에서, Organization/Direct Ask는 Phase 26에서, 실시간 질문방은 Phase 27에서, 태그 상세 정보는 Phase 28에서 모두 백엔드+프론트엔드로 구현되었다** — 각 절의 "백엔드 연동 메모"가 최신 상태를 반영한다. 여전히 남은 격차(`@mention` 자동완성)는 [roadmap.md 7절](roadmap.md#7-백엔드-격차-요약과-착수-전-확인-사항) 참고.
