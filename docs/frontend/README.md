...
이 설계서는 Stack Overflow형 서비스(투표, 댓글, 배지, 모더레이션)를 전제로 2026-08-26에 작성됐고, 당시 Quno 백엔드([PLAN.md](../../PLAN.md))는 다른 방향(리비전, Ward, QPR, Cluster/Super Answer, 평판 점수, Quno Flow)으로 먼저 발전해 있었다. 이후 Phase 11~20에서 이 설계서가 전제한 기능 대부분을 구현했지만, 실제 구현은 목업과 세부가 다른 지점이 많다(예: 모더레이션은 Keep/Hide 두 액션뿐, 댓글 자동완성 없음, 답글은 1단계까지만). 실제 화면/컴포넌트 작업을 시작하기 전에 [roadmap.md의 격차 표](roadmap.md#7-백엔드-격차-요약과-착수-전-확인-사항)와 [design.md](design.md)의 해당 화면 "백엔드 연동 메모"를 먼저 읽고 실제 동작과 목업의 차이를 확인한다.
