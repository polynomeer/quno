...
- [x] E3. Outbox 이벤트가 쌓인 상태(리비전 여러 개를 빠르게 만들어 outbox_events에 미처리 row를 남김)에서 백엔드를 죽였다가 재기동 — 재기동 후 스케줄러가 밀린 이벤트를 마저 처리해 알림이 (지연됐을 뿐) 결국 전부 가는지, 유실되는 게 있는지
...
### E3. Outbox 백로그가 쌓인 상태에서 재기동

- **가설**: `OutboxDispatchScheduler`가 재기동 후에도 쌓여있던 미처리(`published_at IS NULL`) 이벤트를 빠짐없이 처리해 알림이 (지연됐을 뿐) 결국 전부 가야 하고, 유실이나 중복이 없어야 한다.
- **주입 방법**: 실제로 리비전을 빠르게 여러 번 만드는 방식은 스케줄러 주기(2초, 배치 50건)를 실제로 앞지르기 어려워, 백엔드를 정지시킨 채 `outbox_events`에 `published_at IS NULL`인 행 8건을 직접 삽입해 "처리되지 못한 채 쌓인 백로그"를 재현했다(이벤트 타입 `QUESTION_OUTDATED`, `actorId`를 `questionAuthorId`와 같은 값으로 둬 자기 자신에게는 알림이 안 가는 것까지 같이 확인). 이 상태로 백엔드를 재기동.
