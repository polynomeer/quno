...
- [~] 프론트엔드 테스트 실질적 확충 — [production-readiness.md](production-readiness.md) B-5(Phase 36)에서 1→6개 파일로 늘렸지만 전체 커버리지는 여전히 5.5% 수준이었다(도입한 커버리지 도구로 실측). Phase 43~61에 이어 Phase 62(`useRequireAuth`, `useLogin`/`useLogout`, `useSignUp` — 얇은 react-query 래퍼가 아니라 리다이렉트·토큰 저장·회원가입 후 자동 로그인 체이닝 같은 실제 로직이 있는 인증 훅 3종)까지 6→61개 파일, 18→325개 테스트, 라인 커버리지 5.5%→78.57%로 올렸다. 테스트 작성 중 `ForkPanel`/`AnswerCard`의 `mutateAsync` 호출에 try/catch가 빠져 있어 실패 시 unhandled promise rejection이 나는 실제 버그 2건을 발견해 함께 고쳤다. 여전히 지속적 확충이 필요하다(끝나지 않는 항목이라 완료 체크 대신 진행 중으로 표시)
