...
- [x] 25.3 테스트 — 단위 테스트(`CreateDirectAskRequestUseCase`: AWAITING_PAYMENT로 생성, `ConfirmDirectAskPaymentUseCase`: 정상 확인·금액 불일치·미존재 orderId·중복 확인·토스 거부, `RespondToDirectAskRequestUseCase`: 수락 시 결제 유지·거절 시 결제 취소)와 `FakePaymentGateway`. 실제 검증은 두 갈래로 분리했다 — 비즈니스 로직은 위 단위 테스트로, 실제 아웃바운드 HTTP 계약(URL/Basic Auth 헤더/JSON 바디)은 로컬 Python 모크 Toss 서버로 `quno.toss.api-base-url`을 임시 오버라이드해 curl로 전 구간(결제 전 요청 비노출→결제 확인→노출·알림→금액 위조 400→수락 시 결제 유지→거절 시 자동 환불) 확인. **부수 발견**: `RestClient.builder()` 정적 팩토리가 이 프로젝트의 Jackson 3 컨버터 자동구성을 거치지 않아 요청 바디가 조용히 `{}`로 직렬화되던 버그를 모크 서버로 발견, `ObjectMapper` 직접 주입으로 수동 직렬화하도록 수정(ADR-0037 참고). 토스 테스트 모드는 실카드 정보 입력 없이는 진짜 `paymentKey`를 발급하지 않아, 결제위젯을 통한 실제 체크아웃까지는 검증하지 못함 — 사람이 직접 완주해야 하는 갭으로 남김. **이후 별도 검토에서 발견해 수정**: `POST /direct-asks/payments/confirm`이 `@AuthenticationPrincipal`을 받지 않아 호출자가 실제 요청자인지 검증하지 않던 문제를 발견 — `orderId`/`paymentKey`가 사실상 추측 불가능해 실제 악용(금전적 손실)으로 이어지진 않지만, 원칙적으로 소유자 검증이 필요해 `ConfirmDirectAskPaymentCommand`에 `actorId`를 추가하고 `request.requesterId != actorId`면 `DirectAskAccessDeniedException`(403)을 던지도록 수정(회귀 테스트 추가, `DirectAskAccessDeniedException` 메시지도 accept/decline과 공유하도록 일반화)
...
- [x] F16.1 프론트엔드 — `/tags`, `/tags/[name]`, `/organizations`, `/organizations/[id]`, `/users/[id]` 5개 페이지를 `useRequireAuth`에서 `useSession`으로 전환(`/tags`는 아예 인증 훅 자체를 제거 — 액션 UI가 없어 게이팅할 게 없음). 검토 중 발견한 기존 방문자 가드 누락 2건 수정: `FollowUserButton`에 `if (!me) return null` 추가(자기 자신 팔로우 방지 가드만 있고 로그인 여부 가드가 없었음), `EmailDomainVerificationPanel`에 `if (!viewerId) return null` 추가(`viewerId: number | undefined` 시그니처인데 실제 undefined 체크가 없었음). `TagDetailsEditor`/`CreateOrganizationForm`에도 같은 self-guard 신규 추가(기존엔 가드 자체가 없었음 — `TagDetailsEditor`는 설명/문서 링크 자체는 계속 보여주고 "편집" 진입점만 숨김). `JoinOrganizationButton`/`FollowTagButton`/`RequestDirectAskPanel`은 이미 자체 가드가 있어 손대지 않음
...
## Phase 54 — 품질 개선: 프론트엔드 테스트 확충 12 (quality-improvement-plan.md Q-1, 지속 항목)

Phase 53에 이어 같은 지속 항목을 계속 진행한다.

- [x] 54.1 대상 선정 — `DiffView`(질문/답변 리비전 diff 렌더링, 순수 프레젠테이션)와 `QuestionMeta`(작성/수정 시각 표시, `relativeTime` 재사용) — 둘 다 커버리지 0%였다
- [x] 54.2 (시행착오) `DiffView`의 `{prefix} {text}` JSX가 별도 텍스트 노드 3개(EQUAL의 경우 공백 2개 + 본문)로 렌더링돼 `getByText`의 기본 정규화로는 못 찾는 것을 발견 — `container.querySelectorAll` + 수동 공백 정규화로 우회
- [x] 54.3 구현 — `DiffView`는 줄 종류별 접두사(+/-/공백)와 톤 클래스, 빈 diff까지 3개. `QuestionMeta`는 `vi.useFakeTimers`로 상대 시각을 고정해 버전 1(수정 이력 없음)과 버전 3(수정 이력 링크 노출)의 분기를 검증 2개
...
## Phase 57 — 품질 개선: 프론트엔드 테스트 확충 15 (quality-improvement-plan.md Q-1, 지속 항목)

Phase 56에 이어 같은 지속 항목을 계속 진행한다.

- [x] 57.1 대상 선정 — `BadgeList`/`FollowingList`(순수 렌더링 리스트), `DirectAskSettingsToggle`(자기 프로필 전용 온오프 토글), `ReportQueueItem`(모더레이션 큐 항목 — 아직 손대지 않은 모더레이션 도메인 첫 커버리지)
- [x] 57.2 구현 — `BadgeList`/`FollowingList`는 빈 상태+항목 렌더링만 각 2개. `DirectAskSettingsToggle`은 `authApi.updateDirectAskSettings`만 mock해 켜기/끄기 라벨과 토글 호출 4개. `ReportQueueItem`은 `moderationApi.dismiss`/`hide`만 mock — 사유 한글 라벨, QUESTION은 링크·ANSWER는 일반 텍스트로 갈리는 분기(답변 단건 조회 API가 없어 딥링크 불가), Keep/Hide 각각의 뮤테이션 호출까지 6개
...
## Phase 61 — 품질 개선: 프론트엔드 테스트 확충 19 (quality-improvement-plan.md Q-1, 지속 항목)

Phase 60에 이어 같은 지속 항목을 계속 진행한다. 실시간 질문방 도메인의 나머지(UI 계층)를 마저 덮었다.

- [x] 61.1 대상 선정 — `LiveChatPanel`(방 열기/참여 진입점)과 `LiveChatSession`(`@stomp/stompjs`를 실제로 끌어오는 유일한 지점, Phase 39 코드 스플리팅 대상)
- [x] 61.2 구현 — `LiveChatPanel`은 `./LiveChatSession`을 `vi.mock`으로 stub하되 `next/dynamic`은 실제로 동작하게 둬서 지연 로드 경로 자체를 검증(무거운 stompjs 의존성만 피함) — 방 없음/있음에 따른 버튼 분기, 열기 성공 시 즉시 참여, 실패 시 에러 표시까지 5개. `LiveChatSession`은 `useLiveChatMessageHistory`/`useLiveChatSocket`을 훅 레벨 mock(전자는 REST 스크롤백, 후자는 Phase 60에서 이미 별도 검증) — 연결 중/완료 placeholder, 히스토리+실시간 메시지 병합 시 id 기준 중복 제거, 내 메시지("나")/타인 구분, 연결 전 입력 비활성화, Enter로 전송까지 9개
