...
class RespondToDirectAskRequestUseCaseTest {
    ...
    private val createDirectAskRequestUseCase =
        CreateDirectAskRequestUseCase(questionRepository, userRepository, directAskRequestRepository, outboxEventRepository)
    private val useCase = RespondToDirectAskRequestUseCase(directAskRequestRepository, outboxEventRepository)
    ...
    private fun pendingRequest(): Triple<Long, Long, Long> {
        ...
        val requestId = createDirectAskRequestUseCase.execute(
            CreateDirectAskRequestCommand(questionId, requesterId, targetId, null),
        ).id
        return Triple(requestId, requesterId, targetId)
    }

    @Test
    fun `the target can accept, notifying the requester`() {
    ...
    @Test
    fun `the target can decline, notifying the requester`() {
        val (requestId, requesterId, targetId) = pendingRequest()
